...
module.exports = Magix.View.extend({
    ...
    $p += '<div mxa="_zs_galleryaS:_" class="dialog-body"><div mxa="_zs_galleryaS:a" class="_zs_gallery_mx-dialog_alert_-alert-header">' + ($expr = '<%=title%>', $e(title)) + '</div><div mxa="_zs_galleryaS:b" class="word-break">' + ($expr = '<%!content%>', $n(content)) + '</div></div>';
    $line = 5;
    $art = 'if hasBtns';
    ;
    $expr = '<%if (hasBtns) {%>';
    if (hasBtns) {
        ;
        $p += '<div mxa="_zs_galleryaS:c" class="dialog-footer"><a mxa="_zs_galleryaS:d" href="javascript:;" class="btn btn-small btn-brand min-width-60" mx-click="' + $viewId + '@{enter}();">' + ($expr = '<%=enterText%>', $e(enterText)) + '</a></div>';
        $line = 9;
        $art = '/if';
        ;
        $expr = '<%}%>';
