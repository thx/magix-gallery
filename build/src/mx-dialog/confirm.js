...
module.exports = Magix.View.extend({
    ...
    $p += '<div mxa="_zs_gallery`:_" class="dialog-body"><div mxa="_zs_gallery`:a" class="_zs_gallery_mx-dialog_alert_-alert-header">' + ($expr = '<%=title%>', $e(title)) + '</div><div mxa="_zs_gallery`:b" class="word-break">' + ($expr = '<%!content%>', $n(content)) + '</div></div><div mxa="_zs_gallery`:c" class="dialog-footer"><a mxa="_zs_gallery`:d" href="javascript:;" class="btn btn-small btn-brand min-width-50 mr10" mx-click="' + $viewId + '@{enter}();">' + ($expr = '<%=enterText%>', $e(enterText)) + '</a><a mxa="_zs_gallery`:e" href="javascript:;" class="btn btn-small min-width-50" mx-click="' + $viewId + '@{cancel}()">' + ($expr = '<%=cancelText%>', $e(cancelText)) + '</a></div>';
}
catch (ex) {
