...
module.exports = Base.extend({
    ...
    $p += '<div mxa="_zs_gallerycP:_" class="_zs_gallery_mx-main_hor_-steps">';
    $line = 2;
    $art = 'each stepInfos as step';
    ;
    $expr = '<%for (var $art_iushlnndzo$art_i = 0, $art_cpflmlyrvegc$art_c = stepInfos.length; $art_iushlnndzo$art_i < $art_cpflmlyrvegc$art_c; $art_iushlnndzo$art_i++) {    var step = stepInfos[$art_iushlnndzo$art_i]%>';
    for (var $art_iushlnndzo$art_i = 0, $art_cpflmlyrvegc$art_c = stepInfos.length; $art_iushlnndzo$art_i < $art_cpflmlyrvegc$art_c; $art_iushlnndzo$art_i++) {
        var step = stepInfos[$art_iushlnndzo$art_i];
        ...
        ;
        $p += '><div mxa="_zs_gallerycP:a" class="_zs_gallery_mx-main_hor_-circle">';
        $line = 6;
        $art = 'if step.lineOn';
        ;
        $expr = '<%if (step.lineOn) {%>';
        if (step.lineOn) {
            ;
            $p += '<i mxs="_zs_gallerycP:_" class="mc-iconfont _zs_gallery_mx-main_hor_-circle-icon">&#xe65d;</i>';
            $line = 8;
            $art = 'else';
            ;
            $expr = '<%}    else {%>';
        ...
        ;
        $p += '</div><div mxa="_zs_gallerycP:b" class="_zs_gallery_mx-main_hor_-title">';
        $line = 12;
        $art = '=step.label';
        ;
    ...
    $art = '=viewId';
    ;
    $p += ($expr = '<%=viewId%>', $e(viewId)) + '_cur_content"><div mxs="_zs_gallerycP:a" class="loading" style="padding: 150px 0;"><span class="loading-anim"></span></div></div><div>';
    $line = 21;
    ...
    if (curStepInfo.prevTip) {
        ;
        $p += '<a mxa="_zs_gallerycP:c" href="javascript:;" class="btn min-width-100 mr15" mx-click="' + $viewId + 'prev()">';
        $line = 23;
    ...
    if (curStepInfo.nextTip) {
        ;
        $p += '<a mxa="_zs_gallerycP:d" href="javascript:;" class="btn min-width-100 btn-brand" mx-click="' + $viewId + 'next()">';
        $line = 28;
