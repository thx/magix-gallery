...
module.exports = Base.extend({
    ...
    $p += '<div mxa="_zs_gallerych:_" class="_zs_gallery_mx-main_hor_-steps">';
    $line = 2;
    $art = 'each stepInfos as step';
    ;
    $expr = '<%for (var $art_ivdvuoabwp$art_i = 0, $art_ckmunjmxq$art_c = stepInfos.length; $art_ivdvuoabwp$art_i < $art_ckmunjmxq$art_c; $art_ivdvuoabwp$art_i++) {    var step = stepInfos[$art_ivdvuoabwp$art_i]%>';
    for (var $art_ivdvuoabwp$art_i = 0, $art_ckmunjmxq$art_c = stepInfos.length; $art_ivdvuoabwp$art_i < $art_ckmunjmxq$art_c; $art_ivdvuoabwp$art_i++) {
        var step = stepInfos[$art_ivdvuoabwp$art_i];
        ...
        ;
        $p += '><div mxa="_zs_gallerych:a" class="_zs_gallery_mx-main_hor_-circle">';
        $line = 6;
        $art = 'if step.lineOn';
        ;
        $expr = '<%if (step.lineOn) {%>';
        if (step.lineOn) {
            ;
            $p += '<i mxs="_zs_gallerych:_" class="mc-iconfont _zs_gallery_mx-main_hor_-circle-icon">&#xe65d;</i>';
            $line = 8;
            $art = 'else';
            ;
            $expr = '<%}    else {%>';
        ...
        ;
        $p += '</div><div mxa="_zs_gallerych:b" class="_zs_gallery_mx-main_hor_-title">';
        $line = 12;
        $art = '=step.label';
        ;
    ...
    $art = '=viewId';
    ;
    $p += ($expr = '<%=viewId%>', $e(viewId)) + '_cur_content"><div mxs="_zs_gallerych:a" class="loading" style="padding: 150px 0;"><span class="loading-anim"></span></div></div><div>';
    $line = 21;
    ...
    if (curStepInfo.prevTip) {
        ;
        $p += '<a mxa="_zs_gallerych:c" href="javascript:;" class="btn min-width-100 mr15" mx-click="' + $viewId + 'prev()">';
        $line = 23;
    ...
    if (curStepInfo.nextTip) {
        ;
        $p += '<a mxa="_zs_gallerych:d" href="javascript:;" class="btn min-width-100 btn-brand" mx-click="' + $viewId + 'next()">';
        $line = 28;
