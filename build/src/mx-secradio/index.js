...
module.exports = Magix.View.extend({
    ...
    if (needExpand) {
        ;
        $p += '<div mxa="_zs_gallerydu:_" class="_zs_gallery_mx-secradio_index_-line _zs_gallery_mx-secradio_index_-all" mx-click="' + $viewId + '@{toggleAll}()">';
        $line = 3;
        $art = 'if close';
        ;
        $expr = '<%if (close) {%>';
    ...
    for (var itemIndex = 0, $art_cecoznxhfymc$art_c = list.length; itemIndex < $art_cecoznxhfymc$art_c; itemIndex++) {
        ...
        if (needExpand) {
            ;
            $p += '<i mxa="_zs_gallerydu:b" class="mc-iconfont _zs_gallery_mx-secradio_index_-oper">';
            $line = 12;
            $art = 'if item.close';
            ;
            $expr = '<%if (item.close) {%>';
