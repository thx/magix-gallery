...
module.exports = Magix.View.extend({
    ...
    ;
    $expr = '<%for (var $art_ihmdrmd$art_i = 0, $art_cavwp$art_c = options.length; $art_ihmdrmd$art_i < $art_cavwp$art_c; $art_ihmdrmd$art_i++) {    var option = options[$art_ihmdrmd$art_i]%>';
    for (var $art_ihmdrmd$art_i = 0, $art_cavwp$art_c = options.length; $art_ihmdrmd$art_i < $art_cavwp$art_c; $art_ihmdrmd$art_i++) {
        var option = options[$art_ihmdrmd$art_i];
        $p += ' ';
        $line = 14;
        $art = 'each option.params as param index';
        ;
        $expr = '<%for (var index = 0, $art_objpqafdzsnm$art_obj = option.params, $art_cawodflah$art_c = $art_objpqafdzsnm$art_obj.length; index < $art_cawodflah$art_c; index++) {        var param = $art_objpqafdzsnm$art_obj[index]%>';
        for (var index = 0, $art_objpqafdzsnm$art_obj = option.params, $art_cawodflah$art_c = $art_objpqafdzsnm$art_obj.length; index < $art_cawodflah$art_c; index++) {
            var param = $art_objpqafdzsnm$art_obj[index];
            $p += '<tr>';
            $line = 16;
            $art = 'if (index == 0)';
            ;
