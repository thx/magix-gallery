...
module.exports = Magix.View.extend({
    ...
    ;
    $expr = '<%for (var $art_izxurr$art_i = 0, $art_cptdctrpy$art_c = options.length; $art_izxurr$art_i < $art_cptdctrpy$art_c; $art_izxurr$art_i++) {    var option = options[$art_izxurr$art_i]%>';
    for (var $art_izxurr$art_i = 0, $art_cptdctrpy$art_c = options.length; $art_izxurr$art_i < $art_cptdctrpy$art_c; $art_izxurr$art_i++) {
        var option = options[$art_izxurr$art_i];
        $p += ' ';
        $line = 14;
        $art = 'each option.params as param index';
        ;
        $expr = '<%for (var index = 0, $art_objkjjokcim$art_obj = option.params, $art_cdynbnbvau$art_c = $art_objkjjokcim$art_obj.length; index < $art_cdynbnbvau$art_c; index++) {        var param = $art_objkjjokcim$art_obj[index]%>';
        for (var index = 0, $art_objkjjokcim$art_obj = option.params, $art_cdynbnbvau$art_c = $art_objkjjokcim$art_obj.length; index < $art_cdynbnbvau$art_c; index++) {
            var param = $art_objkjjokcim$art_obj[index];
            $p += '<tr>';
            $line = 16;
            $art = 'if (index == 0)';
            ;
