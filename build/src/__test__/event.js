...
module.exports = Magix.View.extend({
    ...
    ;
    $expr = '<%for (var $art_iuidfmic$art_i = 0, $art_cvmtlnpqxj$art_c = options.length; $art_iuidfmic$art_i < $art_cvmtlnpqxj$art_c; $art_iuidfmic$art_i++) {    var option = options[$art_iuidfmic$art_i]%>';
    for (var $art_iuidfmic$art_i = 0, $art_cvmtlnpqxj$art_c = options.length; $art_iuidfmic$art_i < $art_cvmtlnpqxj$art_c; $art_iuidfmic$art_i++) {
        var option = options[$art_iuidfmic$art_i];
        $p += ' ';
        $line = 14;
        $art = 'each option.params as param index';
        ;
        $expr = '<%for (var index = 0, $art_objmrapbxox$art_obj = option.params, $art_cynseyii$art_c = $art_objmrapbxox$art_obj.length; index < $art_cynseyii$art_c; index++) {        var param = $art_objmrapbxox$art_obj[index]%>';
        for (var index = 0, $art_objmrapbxox$art_obj = option.params, $art_cynseyii$art_c = $art_objmrapbxox$art_obj.length; index < $art_cynseyii$art_c; index++) {
            var param = $art_objmrapbxox$art_obj[index];
            $p += '<tr>';
            $line = 16;
            $art = 'if (index == 0)';
            ;
