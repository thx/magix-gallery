...
module.exports = Magix.View.extend({
    ...
    ;
    $expr = '<%for (var $art_itechrpn$art_i = 0, $art_crkfhwke$art_c = options.length; $art_itechrpn$art_i < $art_crkfhwke$art_c; $art_itechrpn$art_i++) {    var option = options[$art_itechrpn$art_i]%>';
    for (var $art_itechrpn$art_i = 0, $art_crkfhwke$art_c = options.length; $art_itechrpn$art_i < $art_crkfhwke$art_c; $art_itechrpn$art_i++) {
        var option = options[$art_itechrpn$art_i];
        $p += ' ';
        $line = 14;
        $art = 'each option.params as param index';
        ;
        $expr = '<%for (var index = 0, $art_objgcikxcgtd$art_obj = option.params, $art_cpcvtnevyb$art_c = $art_objgcikxcgtd$art_obj.length; index < $art_cpcvtnevyb$art_c; index++) {        var param = $art_objgcikxcgtd$art_obj[index]%>';
        for (var index = 0, $art_objgcikxcgtd$art_obj = option.params, $art_cpcvtnevyb$art_c = $art_objgcikxcgtd$art_obj.length; index < $art_cpcvtnevyb$art_c; index++) {
            var param = $art_objgcikxcgtd$art_obj[index];
            $p += '<tr>';
            $line = 16;
            $art = 'if (index == 0)';
            ;
