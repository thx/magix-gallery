...
module.exports = Magix.View.extend({
    ...
    ;
    $expr = '<%for (var $art_iuqhinsas$art_i = 0, $art_clezqvtka$art_c = options.length; $art_iuqhinsas$art_i < $art_clezqvtka$art_c; $art_iuqhinsas$art_i++) {    var option = options[$art_iuqhinsas$art_i]%>';
    for (var $art_iuqhinsas$art_i = 0, $art_clezqvtka$art_c = options.length; $art_iuqhinsas$art_i < $art_clezqvtka$art_c; $art_iuqhinsas$art_i++) {
        var option = options[$art_iuqhinsas$art_i];
        $p += ' ';
        $line = 14;
        $art = 'each option.params as param index';
        ;
        $expr = '<%for (var index = 0, $art_objcxsanqokf$art_obj = option.params, $art_cepmyfgaz$art_c = $art_objcxsanqokf$art_obj.length; index < $art_cepmyfgaz$art_c; index++) {        var param = $art_objcxsanqokf$art_obj[index]%>';
        for (var index = 0, $art_objcxsanqokf$art_obj = option.params, $art_cepmyfgaz$art_c = $art_objcxsanqokf$art_obj.length; index < $art_cepmyfgaz$art_c; index++) {
            var param = $art_objcxsanqokf$art_obj[index];
            $p += '<tr>';
            $line = 16;
            $art = 'if (index == 0)';
            ;
