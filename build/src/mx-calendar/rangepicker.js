...
var Rangepicker = Magix.View.extend({
    ...
    if ($$ref[k = $g + f] === v)
        return k; $$ref[k = $g + $$ref[$g]++] = v; return k; }, viewId = $$.viewId, rangeInfo = $$.rangeInfo, result = $$.result, show = $$.show, left = $$.left, top = $$.top; var $expr, $art, $line; try {
    ...
    if (result.endStr) {
        ;
        $p += '<span mxa="_zs_galleryZ:_" class="_zs_gallery_mx-calendar_rangepicker_-co _zs_gallery_mx-calendar_rangepicker_-co-left">';
        $line = 6;
        $art = '=result.startStr';
        ;
        $p += '' + ($expr = '<%=result.startStr%>', $e(result.startStr)) + '</span><span mxa="_zs_galleryZ:a" class="_zs_gallery_mx-calendar_rangepicker_-center">';
        $line = 7;
        $art = '=result.centetTip';
        ;
        $p += '' + ($expr = '<%=result.centetTip%>', $e(result.centetTip)) + '</span><span mxa="_zs_galleryZ:b" class="_zs_gallery_mx-calendar_rangepicker_-co _zs_gallery_mx-calendar_rangepicker_-co-right">';
        $line = 8;
        $art = '=result.endStr';
        ;
    ...
    $art = '@rangeInfo';
    ;
    $p += '' + ($expr = '<%@rangeInfo%>', $i(rangeInfo)) + '" mx-change="' + $viewId + '@{range.picked}()" mx-cancel="' + $viewId + '@{hide}()"></div>';
}
catch (ex) {
