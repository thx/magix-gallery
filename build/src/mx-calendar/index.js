...
module.exports = Magix.View.extend({
    ...
    $p += '<div mxa="_zs_gallery(:_" class="_zs_gallery_mx-calendar_index_-wrapper mx-shadow unselectable" mx-contextmenu="' + $viewId + '@{prevent}()">';
    $expr = '<%if (types.day) {%>';
    if (types.day) {
        ;
        ...
        if (timeType) {
            ;
            $p += '<div class="clearfix _zs_gallery_mx-calendar_index_-time" mx-change="' + $viewId + '@{setTime}()" mx-view="mx-time/content?time=' + ($expr = '<%!$eu(timeValue)%>', $eu(timeValue)) + '&types=' + ($expr = '<%!$eu(timeType)%>', $eu(timeType)) + '"></div><div mxs="_zs_gallery(:b" class="_zs_gallery_mx-calendar_index_-footer"><button type="button" class="btn btn-small btn-brand" mx-click="' + $viewId + '@{hide}({enter:true})">确定</button><button type="button" class="btn btn-small ml10" mx-click="' + $viewId + '@{hide}()">取消</button></div>';
            $expr = '<%}%>';
        }
        ;
        $p += '</div>';
