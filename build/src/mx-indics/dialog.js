...
module.exports = Magix.View.extend({
    ...
    ;
    $p += '"><div mxa="_zs_gallerycp:_" class="_zs_gallery_mx-indics_index_-header"><span mxs="_zs_gallerycp:_" class="fontsize-16">选择数据字段</span>';
    $line = 4;
    $art = 'if (limit > 0)';
    ;
    $expr = '<%if (limit > 0) {%>';
    if (limit > 0) {
        ;
        $p += '<span mxa="_zs_gallerycp:a" class="color-9 font-tahoma bold ml10">';
        $line = 5;
    ...
    if (!sortable) {
        ;
        $p += '<a mxs="_zs_gallerycp:a" href="javascript:;" class="link-brand fr" mx-click="' + $viewId + 'clear()"><i class="mc-iconfont displacement-2">&#xe72e;</i>清空</a>';
        $line = 11;
        $art = '/if';
        ;
        $expr = '<%}%>';
    ...
    ;
    $p += '</div><div mxv mxa="_zs_gallerycp:b" class="_zs_gallery_mx-indics_index_-content">';
    $line = 14;
    $art = 'each groups as group';
    ;
    $expr = '<%for (var $art_ibtyyvblbp$art_i = 0, $art_ccwzydxl$art_c = groups.length; $art_ibtyyvblbp$art_i < $art_ccwzydxl$art_c; $art_ibtyyvblbp$art_i++) {    var group = groups[$art_ibtyyvblbp$art_i]%>';
    for (var $art_ibtyyvblbp$art_i = 0, $art_ccwzydxl$art_c = groups.length; $art_ibtyyvblbp$art_i < $art_ccwzydxl$art_c; $art_ibtyyvblbp$art_i++) {
        var group = groups[$art_ibtyyvblbp$art_i];
        $p += '<div mxv mxa="_zs_gallerycp:c" class="_zs_gallery_mx-indics_index_-line clearfix">';
        $line = 16;
        $art = 'if group.text';
        ;
        $expr = '<%if (group.text) {%>';
        if (group.text) {
            ;
            $p += '<div mxa="_zs_gallerycp:d" class="_zs_gallery_mx-indics_index_-name">';
            $line = 17;
            $art = '=group.text';
            ;
        ...
        ;
        $p += '<div mxv mxa="_zs_gallerycp:e" class="clearfix">';
        $line = 20;
        $art = 'each group.fields as field';
        ;
        $expr = '<%for (var $art_ixqeivzqztae$art_i = 0, $art_objjolllueq$art_obj = group.fields, $art_chthjwncvc$art_c = $art_objjolllueq$art_obj.length; $art_ixqeivzqztae$art_i < $art_chthjwncvc$art_c; $art_ixqeivzqztae$art_i++) {        var field = $art_objjolllueq$art_obj[$art_ixqeivzqztae$art_i]%>';
        for (var $art_ixqeivzqztae$art_i = 0, $art_objjolllueq$art_obj = group.fields, $art_chthjwncvc$art_c = $art_objjolllueq$art_obj.length; $art_ixqeivzqztae$art_i < $art_chthjwncvc$art_c; $art_ixqeivzqztae$art_i++) {
            var field = $art_objjolllueq$art_obj[$art_ixqeivzqztae$art_i];
    ...
    if (sortable) {
        ;
        $p += '<div mxs="_zs_gallerycp:b" class="_zs_gallery_mx-indics_index_-drag-header"><div class="_zs_gallery_mx-indics_index_-drag-title">自定义字段</div><div class="_zs_gallery_mx-indics_index_-drag-oper"><span class="color-9 mr10">可拖动排序</span><a href="javascript:;" class="link-brand" mx-click="' + $viewId + 'clear()"><i class="mc-iconfont displacement-2">&#xe72e;</i>清空</a></div></div><div mxa="_zs_gallerycp:f" class="_zs_gallery_mx-indics_index_-drag-wrapper" mx-view="mx-dragsort/index" mx-dragfinish="' + $viewId + 'drag()">';
        $line = 49;
        $art = 'each selectedItems as s';
        ;
        $expr = '<%for (var $art_ippcpxiy$art_i = 0, $art_crspfpvm$art_c = selectedItems.length; $art_ippcpxiy$art_i < $art_crspfpvm$art_c; $art_ippcpxiy$art_i++) {        var s = selectedItems[$art_ippcpxiy$art_i]%>';
        for (var $art_ippcpxiy$art_i = 0, $art_crspfpvm$art_c = selectedItems.length; $art_ippcpxiy$art_i < $art_crspfpvm$art_c; $art_ippcpxiy$art_i++) {
            var s = selectedItems[$art_ippcpxiy$art_i];
