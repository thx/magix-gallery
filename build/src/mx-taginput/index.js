...
module.exports = Magix.View.extend({
    ...
    if (items.length) {
        ...
        ;
        $expr = '<%for (var idx = 0, $art_cnyaownxzf$art_c = items.length; idx < $art_cnyaownxzf$art_c; idx++) {        var one = items[idx]%>';
        for (var idx = 0, $art_cnyaownxzf$art_c = items.length; idx < $art_cnyaownxzf$art_c; idx++) {
            var one = items[idx];
    ...
    if (!disabled) {
        ...
        if (loading) {
            ...
            if (suggest && suggest.length) {
                ...
                ;
                $expr = '<%for (var idx = 0, $art_cnevufda$art_c = suggest.length; idx < $art_cnevufda$art_c; idx++) {                var item = suggest[idx]%>';
                for (var idx = 0, $art_cnevufda$art_c = suggest.length; idx < $art_cnevufda$art_c; idx++) {
