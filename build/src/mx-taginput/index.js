...
module.exports = Magix.View.extend({
    ...
    if (items.length) {
        ...
        ;
        $expr = '<%for (var idx = 0, $art_canjaenay$art_c = items.length; idx < $art_canjaenay$art_c; idx++) {        var one = items[idx]%>';
        for (var idx = 0, $art_canjaenay$art_c = items.length; idx < $art_canjaenay$art_c; idx++) {
            var one = items[idx];
    ...
    if (!disabled) {
        ...
        if (loading) {
            ...
            if (suggest && suggest.length) {
                ...
                ;
                $expr = '<%for (var idx = 0, $art_cdorkusvnt$art_c = suggest.length; idx < $art_cdorkusvnt$art_c; idx++) {                var item = suggest[idx]%>';
                for (var idx = 0, $art_cdorkusvnt$art_c = suggest.length; idx < $art_cdorkusvnt$art_c; idx++) {
                ...
                ;
                $p += '<li mxa="_zs_galleryeM:b" class="text-center color-9">';
                $line = 51;
                $art = '=emptyText';
                ;
