...
module.exports = Magix.View.extend({
    ...
    if (items.length) {
        ...
        ;
        $expr = '<%for (var idx = 0, $art_cjutflm$art_c = items.length; idx < $art_cjutflm$art_c; idx++) {        var one = items[idx]%>';
        for (var idx = 0, $art_cjutflm$art_c = items.length; idx < $art_cjutflm$art_c; idx++) {
            var one = items[idx];
    ...
    if (!disabled) {
        ...
        if (loading) {
            ...
            if (suggest && suggest.length) {
                ...
                ;
                $expr = '<%for (var idx = 0, $art_cwpldshoa$art_c = suggest.length; idx < $art_cwpldshoa$art_c; idx++) {                var item = suggest[idx]%>';
                for (var idx = 0, $art_cwpldshoa$art_c = suggest.length; idx < $art_cwpldshoa$art_c; idx++) {
