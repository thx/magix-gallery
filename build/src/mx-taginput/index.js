...
module.exports = Magix.View.extend({
    ...
    if (!items.length) {
        ;
        $p += '<span mxa="_zs_gallerydS:_" class="_zs_gallery_mx-taginput_index_-placeholder">';
        $line = 6;
        $art = '=placeholder';
        ;
    ...
    if (items.length) {
        ...
        ;
        $expr = '<%for (var idx = 0, $art_cokgrxsff$art_c = items.length; idx < $art_cokgrxsff$art_c; idx++) {        var one = items[idx]%>';
        for (var idx = 0, $art_cokgrxsff$art_c = items.length; idx < $art_cokgrxsff$art_c; idx++) {
            var one = items[idx];
            $p += '<span mxa="_zs_gallerydS:a" class="_zs_gallery_mx-taginput_index_-item clearfix" mx-contextmenu="' + $viewId + 'prevent()">';
            $line = 12;
            $art = '=one.text';
            ;
    ...
    if (!disabled) {
        ...
        if (loading) {
            ;
            $p += '<li mxs="_zs_gallerydS:b" class="loading loading-ext _zs_gallery_mx-taginput_index_-loading-small"><span class="loading-anim"></span></li>';
            $line = 40;
            $art = 'else';
            ;
            $expr = '<%}    else {%>';
            ...
            if (suggest && suggest.length) {
                ...
                ;
                $expr = '<%for (var idx = 0, $art_ckgodqggnaj$art_c = suggest.length; idx < $art_ckgodqggnaj$art_c; idx++) {                var item = suggest[idx]%>';
                for (var idx = 0, $art_ckgodqggnaj$art_c = suggest.length; idx < $art_ckgodqggnaj$art_c; idx++) {
                ...
                ;
                $p += '<li mxa="_zs_gallerydS:d" class="text-center color-9">';
                $line = 51;
                $art = '=emptyText';
                ;
