...
module.exports = Magix.View.extend({
    ...
    if ($$ref[k = $g + f] === v)
        return k; $$ref[k = $g + $$ref[$g]++] = v; return k; }, viewId = $$.viewId, items = $$.items, placeholder = $$.placeholder, disabled = $$.disabled, suggest = $$.suggest, width = $$.width, scrollTop = $$.scrollTop; var $expr, $art, $line; try {
    ...
    if (!items.length && placeholder) {
        ;
        $p += '<span mxa="_zs_gallerycl:_" class="_zs_gallery_mx-taginput_index_-placeholder">';
        $line = 3;
        $art = '=placeholder';
        ;
    ...
    if (items.length) {
        ...
        ;
        $p += '';
        $expr = '<%for(var idx=0;idx<items.length;idx++){var one=items[idx]%>';
        for (var idx = 0; idx < items.length; idx++) {
            var one = items[idx];
            $p += '<span mxa="_zs_gallerycl:a" class="_zs_gallery_mx-taginput_index_-item clearfix" mx-contextmenu="' + $viewId + 'prevent()">';
            $line = 9;
            $art = '=one.text';
            ;
    ...
    if (!disabled && suggest && suggest.length) {
        ...
        $art = '@suggest';
        ;
        $p += '' + ($expr = '<%@suggest%>', $i(suggest)) + '&relateIds=ipt_';
        $line = 32;
        $art = '=viewId';
        ;
        ...
        $art = '@scrollTop';
        ;
        $p += '' + ($expr = '<%@scrollTop%>', $i(scrollTop)) + '" class="_zs_gallery_mx-taginput_index_-suggest-wrapper"></div>';
        $line = 36;
        $art = '/if';
        ;
        $p += '';
