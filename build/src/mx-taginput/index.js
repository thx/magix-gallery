...
module.exports = Magix.View.extend({
    ...
    if (!items.length && placeholder) {
        ;
        $p += '<span mxa="_zs_gallerydy:_" class="_zs_gallery_mx-taginput_index_-placeholder">';
        $line = 3;
        $art = '=placeholder';
        ;
    ...
    if (items.length) {
        ...
        ;
        $expr = '<%for (var idx = 0, $art_cpuscefatr$art_c = items.length; idx < $art_cpuscefatr$art_c; idx++) {        var one = items[idx]%>';
        for (var idx = 0, $art_cpuscefatr$art_c = items.length; idx < $art_cpuscefatr$art_c; idx++) {
            var one = items[idx];
            $p += '<span mxa="_zs_gallerydy:a" class="_zs_gallery_mx-taginput_index_-item clearfix" mx-contextmenu="' + $viewId + 'prevent()">';
            $line = 9;
            $art = '=one.text';
            ;
