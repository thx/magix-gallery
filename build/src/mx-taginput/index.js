...
module.exports = Magix.View.extend({
    ...
    if (items.length) {
        ...
        ;
        $expr = '<%for (var idx = 0, $art_czrjxlz$art_c = items.length; idx < $art_czrjxlz$art_c; idx++) {        var one = items[idx]%>';
        for (var idx = 0, $art_czrjxlz$art_c = items.length; idx < $art_czrjxlz$art_c; idx++) {
            var one = items[idx];
    ...
    if (!disabled) {
        ...
        ;
        $p += '" mx-mouseout="' + $viewId + '@{out}()" mx-contextmenu="' + $viewId + '@{prevent}()"><ul mxa="_zs_galleryeY:b" class="mx-output-list">';
        $line = 36;
        $art = 'if loading';
        ;
        $expr = '<%if (loading) {%>';
        if (loading) {
            ...
            if (suggest && suggest.length) {
                ...
                ;
                $expr = '<%for (var idx = 0, $art_cvzbidwxryc$art_c = suggest.length; idx < $art_cvzbidwxryc$art_c; idx++) {                var item = suggest[idx]%>';
                for (var idx = 0, $art_cvzbidwxryc$art_c = suggest.length; idx < $art_cvzbidwxryc$art_c; idx++) {
