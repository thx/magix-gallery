...
module.exports = Magix.View.extend({
    ...
    if (items.length) {
        ...
        ;
        $expr = '<%for (var idx = 0, $art_cnymuezyc$art_c = items.length; idx < $art_cnymuezyc$art_c; idx++) {        var one = items[idx]%>';
        for (var idx = 0, $art_cnymuezyc$art_c = items.length; idx < $art_cnymuezyc$art_c; idx++) {
            var one = items[idx];
    ...
    if (!disabled) {
        ...
        if (loading) {
            ...
            if (suggest && suggest.length) {
                ...
                ;
                $expr = '<%for (var idx = 0, $art_cnpadkhrc$art_c = suggest.length; idx < $art_cnpadkhrc$art_c; idx++) {                var item = suggest[idx]%>';
                for (var idx = 0, $art_cnpadkhrc$art_c = suggest.length; idx < $art_cnpadkhrc$art_c; idx++) {
