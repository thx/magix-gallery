...
module.exports = Magix.View.extend({
    ...
    $p += '<div mxv mxa="_zs_gallerycp:_" class="clearfix _zs_gallery_mx-pagination_index_-pager">';
    $expr = '<%if (!simplify && !mini) {%>';
    if (!simplify && !mini) {
        ;
        $p += '<div mxv mxa="_zs_gallerycp:a" class="_zs_gallery_mx-pagination_index_-infos"><span>' + ($expr = '<%!tipOffset%>', $n(tipOffset)) + '</span><span>' + ($expr = '<%!tipTotal%>', $n(tipTotal)) + '，</span><span>' + ($expr = '<%!tipPer%>', $n(tipPer)) + '</span>';
        $expr = '<%if (sizesChange) {%>';
        if (sizesChange) {
            ;
    ...
    if (!jump) {
        ;
        $p += '<div mxv mxa="_zs_gallerycp:b" class="_zs_gallery_mx-pagination_index_-quick"><span mxa="_zs_gallerycp:c" class="mr5">' + ($expr = '<%=tipJumpTo%>', $e(tipJumpTo)) + '</span><input class="input input-small _zs_gallery_mx-pagination_index_-quick-input mr5" value="';
        $line = 26;
        $art = '=next';
        ;
        ...
        $art = '=viewId';
        ;
        $p += ($expr = '<%=viewId%>', $e(viewId)) + '_jump_input" mx-change="' + $viewId + '@{stop}()" mx-focusin="' + $viewId + '@{stop}()" mx-focusout="' + $viewId + '@{stop}()"/><span mxa="_zs_gallerycp:d" class="mr10">' + ($expr = '<%=tipJumpUnit%>', $e(tipJumpUnit)) + '</span><a mxs="_zs_gallerycp:_" href="javascript:;" class="btn btn-small" mx-click="' + $viewId + '@{jump}()">跳转</a></div>';
        $expr = '<%}%>';
    }
    ;
    ...
    ;
    $p += '><li mxa="_zs_gallerycp:e" class="_zs_gallery_mx-pagination_index_-num-item"><a class="mc-iconfont _zs_gallery_mx-pagination_index_-num-item-icon ';
    $expr = '<%if (page == 1) {%>';
    if (page == 1) {
        ;
    ...
    if (mini) {
        ;
        $p += '<li mxa="_zs_gallerycp:f" class="_zs_gallery_mx-pagination_index_-num-item">' + ($expr = '<%=page%>', $e(page)) + ' / ' + ($expr = '<%=pages%>', $e(pages)) + '</li>';
        $expr = '<%}else {%>';
    }
    else {
        ;
        ...
        if (start > 1) {
            ;
            $p += '<li mxs="_zs_gallerycp:a" class="_zs_gallery_mx-pagination_index_-num-item"><a class="_zs_gallery_mx-pagination_index_-num-item-inner" href="#" mx-click="' + $viewId + '@{toPage}({page:1})">1</a></li>';
            $expr = '<%}%>';
        }
        ;
        $p += ' ';
        ...
        if (start > 2) {
            ;
            $p += '<li mxs="_zs_gallerycp:b" class="_zs_gallery_mx-pagination_index_-num-item"><a class="_zs_gallery_mx-pagination_index_-num-item-inner _zs_gallery_mx-pagination_index_-notallowed" href="#" mx-click="' + $viewId + '@{prevent}()">...</a></li>';
            $expr = '<%}%>';
        }
        ;
        $p += ' ';
        ...
        for (var i = start; i <= end; i++) {
            ;
            $p += '<li mxa="_zs_gallerycp:g" class="_zs_gallery_mx-pagination_index_-num-item"><a class="_zs_gallery_mx-pagination_index_-num-item-inner ';
            $expr = '<%if (i == page) {%>';
            if (i == page) {
                ;
        ...
        if (end < pages) {
            ;
            $p += '<li mxa="_zs_gallerycp:h" class="_zs_gallery_mx-pagination_index_-num-item"><a class="_zs_gallery_mx-pagination_index_-num-item-inner" href="#" mx-click="' + $viewId + '@{toPage}({page:' + ($expr = '<%=pages%>', $e(pages)) + '})">' + ($expr = '<%=pages%>', $e(pages)) + '</a></li>';
            $expr = '<%}%>';
        }
        ;
        $p += ' ';
    ...
    ;
    $p += '<li mxa="_zs_gallerycp:i" class="_zs_gallery_mx-pagination_index_-num-item"><a class="mc-iconfont _zs_gallery_mx-pagination_index_-num-item-icon rotate180 ';
    $expr = '<%if (page == pages) {%>';
    if (page == pages) {
        ;
        $p += '_zs_gallery_mx-pagination_index_-notallowed';
    ...
    if (!hideTotal) {
        ;
        $p += '<li mxa="_zs_gallerycp:j" class="_zs_gallery_mx-pagination_index_-num-item ml10">' + ($expr = '<%!tipTotal%>', $n(tipTotal)) + '</li>';
        $expr = '<%}%>';
    }
    ;
    $p += '</ul></div>';
