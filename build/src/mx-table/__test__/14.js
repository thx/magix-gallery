...
module.exports = Base.extend({
    ...
    $p += '<div mxv mxa="_zs_gallerydP:_" class="_zs_gallery___test___layout_-example"><div mxv mxa="_zs_gallerydP:a" class="_zs_gallery___test___layout_-eg-content"><div mxs="_zs_gallerydP:_" class="clearfix lh22 mb20"><div class="fl color-9">以下示例：</div><div class="fl"><div>使用input value作为主键，<span class="color-brand">请保证input value的唯一性（包含翻页的情况），不要使用index</span></div><div>需要的时候调用<span class="color-brand">this.getStoreState(linkage:string)</span>获取选中值，其中linkage为父节点input.attr(\'linkage\')</div><div>部分禁止选择，翻页保存选中值</div></div></div><div mxa="_zs_gallerydP:b" class="mb20"><a mxs="_zs_gallerydP:a" href="javascript:;" mx-click="' + $viewId + 'clear()" class="btn btn-brand mr20">清空所有选中</a><a mxs="_zs_gallerydP:b" href="javascript:;" mx-click="' + $viewId + 'get()" class="btn btn-brand mr20">获取选中值</a><span mxs="_zs_gallerydP:c" class="color-9">当前选中：</span>';
    $line = 14;
    $art = '=selected';
    ;
    $p += ($expr = '<%=selected%>', $e(selected)) + '</div><div mxv mxa="_zs_gallerydP:c" mx-view="mx-table/index?sticky=true"><div mxv><table mxv mxa="_zs_gallerydP:d" class="table"><thead mxv><tr mxv><th mxv mxa="_zs_gallerydP:e" width="50"><input mxs="_zs_gallerydP:d" type="checkbox" class="checkbox" linkage="example"/></th><th mxs="_zs_gallerydP:e" width="200">列1</th><th mxs="_zs_gallerydP:f" width="200">列2</th><th mxs="_zs_gallerydP:g" width="200">列3</th></tr></thead><tbody mxv>';
    $line = 29;
    $art = 'each list as item index';
    ;
    $expr = '<%for (var index = 0, $art_chxrtxd$art_c = list.length; index < $art_chxrtxd$art_c; index++) {    var item = list[index]%>';
    for (var index = 0, $art_chxrtxd$art_c = list.length; index < $art_chxrtxd$art_c; index++) {
        var item = list[index];
        $p += '<tr mxv class="';
        $line = 30;
    ...
    ;
    $p += '</tbody></table></div></div><div mxa="_zs_gallerydP:f" class="table-pager-wrapper"><div mx-change="' + $viewId + 'changePager()" mx-view="mx-pagination/index?total=';
    $line = 47;
    $art = '=total';
    ;
    ...
    $art = '=page';
    ;
    $p += ($expr = '<%!$eu(page)%>', $eu(page)) + '"></div></div></div><div mxa="_zs_gallerydP:g" class="clearfix"><div mxa="_zs_gallerydP:h" class="_zs_gallery___test___layout_-eg-desc _zs_gallery___test___layout_-half"><div mxs="_zs_gallerydP:h" class="_zs_gallery___test___layout_-eg-title">HTML Code</div><div class="_zs_gallery___test___layout_-desc-oper" mx-success="' + $viewId + 'done({id:1})" mx-view="mx-copy/index?copyNode=';
    $line = 56;
    $art = '=viewId';
    ;
    ...
    $art = '!text2';
    ;
    $p += ($expr = '<%!text2%>', $n(text2)) + '</span><i mxs="_zs_gallerydP:i" class="mc-iconfont _zs_gallery___test___layout_-desc-icon">&#xe610;</i></div><pre mx-view="__test__/hl" id="';
    $line = 116;
    $art = '=viewId';
    ;
