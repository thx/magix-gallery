...
module.exports = Base.extend({
    ...
    $p += '<div mxa="_zs_gallerydk:_" class="_zs_gallery_mx-status_base_-status" mx-mouseout="' + $viewId + 'out()"><div mxa="_zs_gallerydk:a" class="_zs_gallery_mx-status_base_-status-handle-icon" mx-mouseover="' + $viewId + 'over()">';
    $line = 5;
    $art = 'if showInfo';
    ;
    $expr = '<%if (showInfo) {%>';
    ...
    if (opers.length > 0) {
        ;
        $p += '<i mxs="_zs_gallerydk:_" class="_zs_gallery_mx-status_base_-status-arrow"></i>';
        $line = 13;
        $art = '/if';
        ;
        $expr = '<%}%>';
    ...
    if (show) {
        ;
        $p += '<ul mxa="_zs_gallerydk:b" class="_zs_gallery_mx-status_base_-status-info">';
        $line = 18;
        $art = 'if showInfo';
        ;
        $expr = '<%if (showInfo) {%>';
        if (showInfo) {
            ;
            $p += '<li mxa="_zs_gallerydk:c" class="_zs_gallery_mx-status_base_-status-desc"><span class="_zs_gallery_mx-status_base_-status-icon" style="color: ';
            $line = 20;
            $art = '=info.color';
            ;
            $p += ($expr = '<%=info.color%>', $e(info.color)) + ';">';
            ...
            $art = '!info.icon';
            ;
            $p += ($expr = '<%!info.icon%>', $n(info.icon)) + '</span><span mxa="_zs_gallerydk:d" class="_zs_gallery_mx-status_base_-status-name">';
            $line = 21;
            $art = '!info.text';
            ;
            ...
            if (info.tip) {
                ;
                $p += '<li mxa="_zs_gallerydk:e" class="_zs_gallery_mx-status_base_-status-tip">';
                $line = 24;
                $art = '!info.tip';
                ;
            ...
            if (opers.length > 0) {
                ;
                $p += '<li mxs="_zs_gallerydk:a" class="_zs_gallery_mx-status_base_-status-line"></li>';
                $line = 28;
                $art = '/if';
                ;
                $expr = '<%}%>';
        ...
        ;
        $expr = '<%for (var $art_iqbwrnbo$art_i = 0, $art_cfbthigtd$art_c = opers.length; $art_iqbwrnbo$art_i < $art_cfbthigtd$art_c; $art_iqbwrnbo$art_i++) {        var item = opers[$art_iqbwrnbo$art_i]%>';
        for (var $art_iqbwrnbo$art_i = 0, $art_cfbthigtd$art_c = opers.length; $art_iqbwrnbo$art_i < $art_cfbthigtd$art_c; $art_iqbwrnbo$art_i++) {
            var item = opers[$art_iqbwrnbo$art_i];
            ...
            $art = '!item.icon';
            ;
            $p += ($expr = '<%!item.icon%>', $n(item.icon)) + '</span><span mxa="_zs_gallerydk:f" class="_zs_gallery_mx-status_base_-status-name">';
            $line = 34;
            $art = '!item.text';
            ;
            $p += ($expr = '<%!item.text%>', $n(item.text)) + '</span><i mxs="_zs_gallerydk:b" class="mc-iconfont _zs_gallery_mx-status_base_-oper-icon">&#xe65d;</i></li>';
            $line = 37;
            $art = '/each';
            ;
            $expr = '<%}%>';
