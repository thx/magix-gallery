...
module.exports = Base.extend({
    ...
    $p += '<div mxa="_zs_galleryc7:_" class="_zs_gallery_mx-status_base_-status" mx-mouseout="' + $viewId + 'out()"><div mxa="_zs_galleryc7:a" class="_zs_gallery_mx-status_base_-status-handle-dd" mx-mouseover="' + $viewId + 'over()">';
    $line = 5;
    $art = 'if showInfo';
    ;
    $expr = '<%if (showInfo) {%>';
    if (showInfo) {
        ...
        $art = '=cur.text';
        ;
        $p += ($expr = '<%=cur.text%>', $e(cur.text)) + '</span><i mxs="_zs_galleryc7:_" class="mc-iconfont _zs_gallery_mx-status_base_-status-arrow">&#xe692;</i>';
        $line = 10;
        $art = '/if';
        ;
        $expr = '<%}%>';
    ...
    if (show && !showInfo && (opers.length > 0)) {
        ;
        $p += '<ul mxa="_zs_galleryc7:b" class="_zs_gallery_mx-status_base_-status-info _zs_gallery_mx-status_base_-status-info-dd">';
        $line = 16;
        $art = 'each opers as item';
        ;
        $expr = '<%for (var $art_ifamsi$art_i = 0, $art_cjjswea$art_c = opers.length; $art_ifamsi$art_i < $art_cjjswea$art_c; $art_ifamsi$art_i++) {        var item = opers[$art_ifamsi$art_i]%>';
        for (var $art_ifamsi$art_i = 0, $art_cjjswea$art_c = opers.length; $art_ifamsi$art_i < $art_cjjswea$art_c; $art_ifamsi$art_i++) {
            var item = opers[$art_ifamsi$art_i];
            ...
            $art = '!item.icon';
            ;
            $p += ($expr = '<%!item.icon%>', $n(item.icon)) + '</span><span mxa="_zs_galleryc7:c" class="_zs_gallery_mx-status_base_-status-name">';
            $line = 19;
            $art = '=item.text';
            ;
            $p += ($expr = '<%=item.text%>', $e(item.text)) + '</span><i mxs="_zs_galleryc7:a" class="mc-iconfont _zs_gallery_mx-status_base_-oper-icon">&#xe65d;</i></li>';
            $line = 22;
            $art = '/each';
            ;
            $expr = '<%}%>';
