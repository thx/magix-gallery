...
module.exports = Base.extend({
    ...
    $p += '<div mxa="_zs_galleryd_:_" class="_zs_gallery_mx-status_base_-status" mx-mouseout="' + $viewId + 'out()"><div mxa="_zs_galleryd_:a" class="_zs_gallery_mx-status_base_-status-handle-dd" mx-mouseover="' + $viewId + 'over()">';
    $line = 5;
    $art = 'if showInfo';
    ;
    $expr = '<%if (showInfo) {%>';
    if (showInfo) {
        ...
        $art = '=cur.text';
        ;
        $p += ($expr = '<%=cur.text%>', $e(cur.text)) + '</span><i mxs="_zs_galleryd_:_" class="mc-iconfont _zs_gallery_mx-status_base_-status-arrow">&#xe692;</i>';
        $line = 10;
        $art = '/if';
        ;
        $expr = '<%}%>';
    ...
    if (show && !showInfo && (opers.length > 0)) {
        ;
        $p += '<ul mxa="_zs_galleryd_:b" class="_zs_gallery_mx-status_base_-status-info _zs_gallery_mx-status_base_-status-info-dd">';
        $line = 16;
        $art = 'each opers as item';
        ;
        $expr = '<%for (var $art_iqngrvhl$art_i = 0, $art_cfrhzdqm$art_c = opers.length; $art_iqngrvhl$art_i < $art_cfrhzdqm$art_c; $art_iqngrvhl$art_i++) {        var item = opers[$art_iqngrvhl$art_i]%>';
        for (var $art_iqngrvhl$art_i = 0, $art_cfrhzdqm$art_c = opers.length; $art_iqngrvhl$art_i < $art_cfrhzdqm$art_c; $art_iqngrvhl$art_i++) {
            var item = opers[$art_iqngrvhl$art_i];
            ...
            $art = '!item.icon';
            ;
            $p += ($expr = '<%!item.icon%>', $n(item.icon)) + '</span><span mxa="_zs_galleryd_:c" class="_zs_gallery_mx-status_base_-status-name">';
            $line = 19;
            $art = '=item.text';
            ;
            $p += ($expr = '<%=item.text%>', $e(item.text)) + '</span><i mxs="_zs_galleryd_:a" class="mc-iconfont _zs_gallery_mx-status_base_-oper-icon">&#xe65d;</i></li>';
            $line = 22;
            $art = '/each';
            ;
            $expr = '<%}%>';
