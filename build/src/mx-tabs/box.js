...
module.exports = Base.extend({
    ...
    ;
    $expr = '<%for (var index = 0, $art_cfchmpljjwp$art_c = list.length; index < $art_cfchmpljjwp$art_c; index++) {    var item = list[index]%>';
    for (var index = 0, $art_cfchmpljjwp$art_c = list.length; index < $art_cfchmpljjwp$art_c; index++) {
        var item = list[index];
        $p += '<a href="javascript:;" ';
        $line = 3;
        $art = 'if !disabled';
        ...
        if (item.tag) {
            ...
            $art = '=item.color';
            ;
            $p += ($expr = '<%=item.color%>', $e(item.color)) + ';"></span><span mxa="_zs_galleryeW:_" class="mx-tag-name">';
            $line = 9;
            $art = '!item.tag';
            ;
