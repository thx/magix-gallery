...
module.exports = Base.extend({
    ...
    ;
    $expr = '<%for (var index = 0, $art_cldbltnkcvk$art_c = list.length; index < $art_cldbltnkcvk$art_c; index++) {    var item = list[index]%>';
    for (var index = 0, $art_cldbltnkcvk$art_c = list.length; index < $art_cldbltnkcvk$art_c; index++) {
        var item = list[index];
        $p += '<a href="javascript:;" ';
        $line = 3;
        $art = 'if !disabled';
        ...
        if (item.tag) {
            ;
            $p += '<span mxa="_zs_galleryd0:_" class="_zs_gallery_mx-tabs_index_-tag">';
            $line = 7;
            $art = '=item.tag';
            ;
