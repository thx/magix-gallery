...
module.exports = Base.extend({
    ...
    ;
    $p += '';
    $expr = '<%for(var $art_ivymwoifza$art_i=0;$art_ivymwoifza$art_i<list.length;$art_ivymwoifza$art_i++){var item=list[$art_ivymwoifza$art_i]%>';
    for (var $art_ivymwoifza$art_i = 0; $art_ivymwoifza$art_i < list.length; $art_ivymwoifza$art_i++) {
        var item = list[$art_ivymwoifza$art_i];
        $p += '<a href="javascript:;" ';
        $line = 3;
        $art = 'if !disabled';
        ;
        ...
        if (item.tag) {
            ;
            $p += '<span mxa="_zs_galleryci:_" class="_zs_gallery_mx-tabs_index_-tag">';
            $line = 6;
            $art = '=item.tag';
            ;
