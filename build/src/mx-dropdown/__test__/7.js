...
module.exports = Base.extend({
    ...
    $p += '<div mxa="_zs_galleryaQ:_" class="_zs_gallery___test___layout_-example"><div mxa="_zs_galleryaQ:a" class="_zs_gallery___test___layout_-eg-content"><div mxs="_zs_galleryaQ:_" class="mb20 clearfix lh22"><div class="fl color-9">以下示例：</div><div class="fl"><div>带分组的只能用mx-dropdown.item节点实现</div><div>不需要全选功能，配置need-all="false"</div><div>组上面需要全选功能在mx-dropdown.item上配置all="true"</div><div>展开选择浮层时会修正浮层到可视范围之内</div></div></div><div mxa="_zs_galleryaQ:b" class="clearfix"><div class="w200 fl" mx-view="mx-dropdown/multiple?needAll=false&searchbox=true&name=%E6%95%B0%E5%AD%97&emptyText=%E8%AF%B7%E9%80%89%E6%8B%A9%E6%95%B0%E5%AD%97&selected=';
    $line = 18;
    $art = '=selected';
    ;
    $p += ($expr = '<%!$eu(selected)%>', $eu(selected)) + '"><i mxs="_zs_galleryaQ:a" group="true" all="true" class="none">第一组</i>';
    $line = 20;
    $art = 'each first as i';
    ;
    $expr = '<%for (var $art_ibrgppmhtig$art_i = 0, $art_chpazwsidk$art_c = first.length; $art_ibrgppmhtig$art_i < $art_chpazwsidk$art_c; $art_ibrgppmhtig$art_i++) {    var i = first[$art_ibrgppmhtig$art_i]%>';
    for (var $art_ibrgppmhtig$art_i = 0, $art_chpazwsidk$art_c = first.length; $art_ibrgppmhtig$art_i < $art_chpazwsidk$art_c; $art_ibrgppmhtig$art_i++) {
        var i = first[$art_ibrgppmhtig$art_i];
        $p += '<i value="';
        $line = 21;
        $art = '=i';
        ;
    ...
    ;
    $p += '<i mxs="_zs_galleryaQ:b" group="true" all="true" class="none">第二组</i>';
    $line = 24;
    $art = 'each second as i';
    ;
    $expr = '<%for (var $art_ifmnqdlyem$art_i = 0, $art_ccpleojcl$art_c = second.length; $art_ifmnqdlyem$art_i < $art_ccpleojcl$art_c; $art_ifmnqdlyem$art_i++) {    var i = second[$art_ifmnqdlyem$art_i]%>';
    for (var $art_ifmnqdlyem$art_i = 0, $art_ccpleojcl$art_c = second.length; $art_ifmnqdlyem$art_i < $art_ccpleojcl$art_c; $art_ifmnqdlyem$art_i++) {
        var i = second[$art_ifmnqdlyem$art_i];
        $p += '<i value="';
        $line = 25;
        $art = '=i';
        ;
