...
module.exports = Magix.View.extend({
    ...
    ;
    $p += '"><span mxa="_zs_gallerya9:_" class="_zs_gallery_mx-dropdown_index_-dropdown-toggle-label">';
    $expr = '<%if (name) {%>';
    if (name) {
        ;
        $p += '<span mxa="_zs_gallerya9:a" class="color-9">' + ($expr = '<%=name%>', $e(name)) + '：</span>';
        $expr = '<%}%>';
    }
    ;
    $p += ' ' + ($expr = '<%=selectedText%>', $e(selectedText)) + '</span><span mxs="_zs_gallerya9:_" class="mc-iconfont _zs_gallery_mx-dropdown_index_-arrow">&#xe692;</span></div><div mxv class="_zs_gallery_mx-dropdown_index_-dropdown-menu-wrapper ' + ($expr = '<%=placementClass%>', $e(placementClass)) + ' ';
    $expr = '<%if (expand) {%>';
    if (expand) {
        ;
        $p += ' _zs_gallery_mx-dropdown_index_-open ';
