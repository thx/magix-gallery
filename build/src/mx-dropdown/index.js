...
module.exports = Magix.View.extend({
    ...
    if (name) {
        ;
        $p += '<span mxa="_zs_galleryaY:_" class="color-9">' + ($expr = '<%=name%>', $e(name)) + '：</span>';
        $expr = '<%}%>';
    }
    ;
    $p += ' ' + ($expr = '<%=selectedText%>', $e(selectedText)) + '</span><span mxs="_zs_galleryaY:_" class="mc-iconfont _zs_gallery_mx-dropdown_index_-arrow">&#xe692;</span></div><div mxv class="_zs_gallery_mx-dropdown_index_-dropdown-menu-wrapper ' + ($expr = '<%=placementClass%>', $e(placementClass)) + ' ';
    $expr = '<%if (expand) {%>';
    if (expand) {
        ;
        $p += ' _zs_gallery_mx-dropdown_index_-open ';
