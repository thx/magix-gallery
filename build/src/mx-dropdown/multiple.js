...
module.exports = Magix.View.extend({
    ...
    if (rList) {
        ...
        if (searchbox) {
            ;
            $p += '<div mxv mxa="_zs_galleryaT:_" class="_zs_gallery_mx-dropdown_index_-search-wrapper"><div mxv mxa="_zs_galleryaT:a" class="search-box _zs_gallery_mx-dropdown_index_-dropdown-search-box"><i mxs="_zs_galleryaT:a" class="mc-iconfont search-icon">&#xe651;</i><input class="input search-input" placeholder="';
            $line = 13;
            $art = '=text.search';
            ;
            ...
            if (max > 0 && over) {
                ;
                $p += '<span mxa="_zs_galleryaT:b" class="ml10"><span mxs="_zs_galleryaT:b" class="color-9">已选：</span><span mxa="_zs_galleryaT:c" class="font-tahoma bold">';
                $line = 24;
                $art = '=imme.length';
                ;
        ...
        if (!needAll || (max > 0 && !over)) {
            ...
            if (max > 0 && !over) {
                ...
                ;
                $p += '><span mxs="_zs_galleryaT:b" class="color-9">已选：</span><span mxa="_zs_galleryaT:d" class="font-tahoma bold">';
                $line = 39;
                $art = '=imme.length';
                ;
                $p += ($expr = '<%=imme.length%>', $e(imme.length)) + ' / ';
        ...
        ;
        $expr = '<%for (var groupIndex = 0, $art_cvylwsdtgp$art_c = groups.length; groupIndex < $art_cvylwsdtgp$art_c; groupIndex++) {        var group = groups[groupIndex]%>';
        for (var groupIndex = 0, $art_cvylwsdtgp$art_c = groups.length; groupIndex < $art_cvylwsdtgp$art_c; groupIndex++) {
            ...
            ;
            $p += '<div mxv mxa="_zs_galleryaT:e" class="clearfix _zs_gallery_mx-dropdown_index_-group-wrapper">';
            $line = 64;
            $art = 'each group.list as item';
            ;
            $expr = '<%for (var $art_iyoqqttjvc$art_i = 0, $art_objeqxspubwif$art_obj = group.list, $art_cwrvzgs$art_c = $art_objeqxspubwif$art_obj.length; $art_iyoqqttjvc$art_i < $art_cwrvzgs$art_c; $art_iyoqqttjvc$art_i++) {            var item = $art_objeqxspubwif$art_obj[$art_iyoqqttjvc$art_i]%>';
            for (var $art_iyoqqttjvc$art_i = 0, $art_objeqxspubwif$art_obj = group.list, $art_cwrvzgs$art_c = $art_objeqxspubwif$art_obj.length; $art_iyoqqttjvc$art_i < $art_cwrvzgs$art_c; $art_iyoqqttjvc$art_i++) {
                var item = $art_objeqxspubwif$art_obj[$art_iyoqqttjvc$art_i];
                $p += ' ';
                $line = 65;
                $art = 'if !item.hide';
                ;
        ...
        ;
        $p += '</div><div mxa="_zs_galleryaT:f" class="_zs_gallery_mx-dropdown_index_-footer-wrapper"><button mxa="_zs_galleryaT:g" type="button" class="btn btn-small btn-brand mr10" mx-click="' + $viewId + '@{submit}({enter:true})">';
        $line = 83;
        $art = '=text.submit';
        ;
        $p += ($expr = '<%=text.submit%>', $e(text.submit)) + '</button><button mxa="_zs_galleryaT:h" type="button" class="btn btn-small" mx-click="' + $viewId + '@{hide}()">';
        $line = 84;
        $art = '=text.cancel';
        ;
