...
module.exports = Magix.View.extend({
    ...
    ;
    $p += '';
    $expr = '<%for (var index = 0, $art_ccpvymyenj$art_c = list.length; index < $art_ccpvymyenj$art_c; index++) {    var item = list[index]%>';
    for (var index = 0, $art_ccpvymyenj$art_c = list.length; index < $art_ccpvymyenj$art_c; index++) {
        var item = list[index];
        $p += '<div mxv mxa="_zs_gallerycS:_" class="_zs_gallery_mx-tree_index_-li"><div mxv mxa="_zs_gallerycS:a" class="_zs_gallery_mx-tree_index_-name clearfix">';
        $line = 4;
        $art = 'if needExpand';
        ;
        $p += '';
        ...
        if (needExpand) {
            ...
            if (item.children && (item.children.length > 0)) {
                ...
                if (readOnly) {
                    ;
                    $p += '<i mxs="_zs_gallerycS:_" class="mc-iconfont _zs_gallery_mx-tree_index_-icon empty">&#xe732;</i>';
                    $line = 17;
                    $art = '/if';
                    ;
                    $p += '';
        ...
        ;
        $p += '<label mxv mxa="_zs_gallerycS:b" class="fl">';
        $line = 21;
        $art = 'if !readOnly';
        ;
        $p += '';
