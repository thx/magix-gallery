...
module.exports = Magix.View.extend({
    ...
    ;
    $expr = '<%for (var index = 0, $art_cmrozlquvcq$art_c = list.length; index < $art_cmrozlquvcq$art_c; index++) {    var item = list[index]%>';
    for (var index = 0, $art_cmrozlquvcq$art_c = list.length; index < $art_cmrozlquvcq$art_c; index++) {
        var item = list[index];
        $p += '<div mxv mxa="_zs_galleryeZ:_" class="_zs_gallery_mx-tree_index_-li"><div mxv mxa="_zs_galleryeZ:a" class="_zs_gallery_mx-tree_index_-name clearfix">';
        $line = 4;
        $art = 'if needExpand';
        ;
        $expr = '<%if (needExpand) {%>';
        if (needExpand) {
            ...
            if (item.children && (item.children.length > 0)) {
                ...
                if (readOnly) {
                    ;
                    $p += '<i mxs="_zs_galleryeZ:_" class="mc-iconfont _zs_gallery_mx-tree_index_-icon empty">&#xe732;</i>';
                    $line = 17;
                    $art = '/if';
                    ;
                    $expr = '<%}%>';
        ...
        ;
        $p += '<label mxv mxa="_zs_galleryeZ:b" class="fl">';
        $line = 21;
        $art = 'if !readOnly';
        ;
        $expr = '<%if (!readOnly) {%>';
