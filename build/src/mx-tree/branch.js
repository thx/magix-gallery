...
module.exports = Magix.View.extend({
    ...
    ;
    $expr = '<%for (var index = 0, $art_cpwbqqmmrf$art_c = list.length; index < $art_cpwbqqmmrf$art_c; index++) {    var item = list[index]%>';
    for (var index = 0, $art_cpwbqqmmrf$art_c = list.length; index < $art_cpwbqqmmrf$art_c; index++) {
        var item = list[index];
        ...
        if (needExpand) {
            ...
            if (item.children && (item.children.length > 0)) {
                ...
                if (readOnly) {
                    ;
                    $p += '<i mxs="_zs_gallerye4:_" class="mc-iconfont _zs_gallery_mx-tree_index_-icon">&#xe732;</i>';
                    $line = 13;
                    $art = '/if';
                    ;
                    $expr = '<%}%>';
