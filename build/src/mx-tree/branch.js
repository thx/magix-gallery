...
module.exports = Magix.View.extend({
    ...
    ;
    $expr = '<%for (var index = 0, $art_czzozwmshb$art_c = list.length; index < $art_czzozwmshb$art_c; index++) {    var item = list[index]%>';
    for (var index = 0, $art_czzozwmshb$art_c = list.length; index < $art_czzozwmshb$art_c; index++) {
        var item = list[index];
        $p += '<div mxv mxa="_zs_galleryd;:_" class="_zs_gallery_mx-tree_index_-li"><div mxv mxa="_zs_galleryd;:a" class="_zs_gallery_mx-tree_index_-name clearfix">';
        $line = 4;
        $art = 'if needExpand';
        ;
        $expr = '<%if (needExpand) {%>';
        if (needExpand) {
            ...
            if (item.children && (item.children.length > 0)) {
                ...
                if (readOnly) {
                    ;
                    $p += '<i mxs="_zs_galleryd;:_" class="mc-iconfont _zs_gallery_mx-tree_index_-icon empty">&#xe732;</i>';
                    $line = 17;
                    $art = '/if';
                    ;
                    $expr = '<%}%>';
        ...
        ;
        $p += '<label mxv mxa="_zs_galleryd;:b" class="fl">';
        $line = 21;
        $art = 'if !readOnly';
        ;
        $expr = '<%if (!readOnly) {%>';
