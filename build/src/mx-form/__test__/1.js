...
module.exports = Base.extend({
    ...
    ;
    $p += '';
    $expr = '<%for (var index = 0, $art_copyaz$art_c = list.length; index < $art_copyaz$art_c; index++) {    var item = list[index]%>';
    for (var index = 0, $art_copyaz$art_c = list.length; index < $art_copyaz$art_c; index++) {
        var item = list[index];
        $p += '<tr mxv><td>';
        $line = 36;
        $art = '=item.name';
