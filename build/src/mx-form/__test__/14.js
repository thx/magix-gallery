...
module.exports = Base.extend({
    ...
    $p += '<div mxv mxa="_zs_gallerya1:_" class="_zs_gallery___test___base_-example"><div mxv mxa="_zs_gallerya1:a" class="_zs_gallery___test___base_-eg-content"><div mxs="_zs_gallerya1:_" class="mb20"><span class="color-9 mr20">动态添加（要求2 ~ 4个字符）：</span><a href="javascript:;" class="link-brand" mx-click="' + $viewId + 'add()">+ 再加一条</a></div><div mxv>';
    $line = 9;
    $art = 'each list as item';
    ;
    $p += '';
    $expr = '<%for(var $art_isljoqhn$art_i=0;$art_isljoqhn$art_i<list.length;$art_isljoqhn$art_i++){var item=list[$art_isljoqhn$art_i]%>';
    for (var $art_isljoqhn$art_i = 0; $art_isljoqhn$art_i < list.length; $art_isljoqhn$art_i++) {
        var item = list[$art_isljoqhn$art_i];
        $p += '<input mxe="' + $viewId + '_0_' + ($expr = '<%=$art_isljoqhn$art_i%>', $e($art_isljoqhn$art_i)) + '" mxc="[';
        $line = 11;
        $art = ':item.value{required:true,blength:[2,4]}';
        ;
        $p += '{p:\'list.' + ($expr = '<%=$art_isljoqhn$art_i%>', $e($art_isljoqhn$art_i)) + '.value\',f:{required:true,blength:[2,4]}}]" class="input mr20 mb20 w120" placeholder="请填写" value="';
        $line = 11;
        $art = ':item.value{required:true,blength:[2,4]}';
        ;
    ...
    ;
    $p += '</div></div><div mxa="_zs_gallerya1:b" class="clearfix"><div mxa="_zs_gallerya1:c" class="_zs_gallery___test___base_-eg-desc _zs_gallery___test___base_-half"><div mxs="_zs_gallerya1:a" class="_zs_gallery___test___base_-eg-title">HTML Code</div><div class="_zs_gallery___test___base_-desc-oper" mx-success="' + $viewId + 'done({id:1})" mx-view="mx-copy/index?copyNode=';
    $line = 18;
    $art = '=viewId';
    ;
    $p += '' + ($expr = '<%!$eu(viewId)%>', $eu(viewId)) + '_text_1"><span mxa="_zs_gallerya1:d" class="_zs_gallery___test___base_-desc-tip">';
    $line = 20;
    $art = '!text1';
    ;
