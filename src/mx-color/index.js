...
module.exports = Magix.View.extend({
    ...
    $p += '<div id="scs_' + ($expr = '<%=id%>', $e(id)) + '"><ul mxa="_2:_" class="__mx-color_index_-shortcuts">';
    $expr = '<%for (var i = 0; i < shortcuts.length; i++) {%>';
    for (var i = 0; i < shortcuts.length; i++) {
        ;
        $p += '<li class="__mx-color_index_-shortcuts-item __mx-style_index_-cp" id="sc_' + ($expr = '<%=shortcuts[i]%>', $e(shortcuts[i])) + '_' + ($expr = '<%=id%>', $e(id)) + '" style="background:#' + ($expr = '<%=shortcuts[i]%>', $e(shortcuts[i])) + '" mx-click="' + $viewId + '@{shortcuts.picked}({color:\'#' + ($expr = '<%=$eq(shortcuts[i])%>', $e($eq(shortcuts[i]))) + '\'})"></li>';
        $expr = '<%}%>';
    }
    ;
    $p += '</ul></div><div mxa="_2:a" class="__mx-color_index_-body"><div class="__mx-color_index_-cpicker __mx-style_index_-cp" id="cz_' + ($expr = '<%=id%>', $e(id)) + '" mx-mousedown="' + $viewId + '@{color.zone.drag}()"><svg mxa="_2:b" xmlns="http://www.w3.org/2000/svg" version="1.1" width="100%" height="100%"><defs><lineargradient id="gb_' + ($expr = '<%=id%>', $e(id)) + '" x1="0%" y1="100%" x2="0%" y2="0%"><stop mxs="_2:_" offset="0%" stop-color="#000000" stop-opacity="1"></stop><stop mxs="_2:a" offset="100%" stop-color="#CC9A81" stop-opacity="0"></stop></lineargradient><lineargradient id="gw_' + ($expr = '<%=id%>', $e(id)) + '" x1="0%" y1="100%" x2="100%" y2="100%"><stop mxs="_2:b" offset="0%" stop-color="#FFFFFF" stop-opacity="1"></stop><stop mxs="_2:a" offset="100%" stop-color="#CC9A81" stop-opacity="0"></stop></lineargradient></defs><rect x="0" y="0" width="100%" height="100%" fill="url(#gw_' + ($expr = '<%=id%>', $e(id)) + ')"></rect><rect x="0" y="0" width="100%" height="100%" fill="url(#gb_' + ($expr = '<%=id%>', $e(id)) + ')"></rect></svg><div class="__mx-color_index_-cpicker-indicator" id="ci_' + ($expr = '<%=id%>', $e(id)) + '"></div></div><div mxa="_2:c" class="__mx-color_index_-slide __mx-style_index_-cp" mx-mousedown="' + $viewId + '@{slide.drag}()"><svg mxa="_2:d" xmlns="http://www.w3.org/2000/svg" version="1.1" width="100%" height="100%"><defs><lineargradient id="ghsv_' + ($expr = '<%=id%>', $e(id)) + '" x1="0%" y1="100%" x2="0%" y2="0%"><stop mxs="_2:c" offset="0%" stop-color="#FF0000" stop-opacity="1"></stop><stop mxs="_2:d" offset="13%" stop-color="#FF00FF" stop-opacity="1"></stop><stop mxs="_2:e" offset="25%" stop-color="#8000FF" stop-opacity="1"></stop><stop mxs="_2:f" offset="38%" stop-color="#0040FF" stop-opacity="1"></stop><stop mxs="_2:g" offset="50%" stop-color="#00FFFF" stop-opacity="1"></stop><stop mxs="_2:h" offset="63%" stop-color="#00FF40" stop-opacity="1"></stop><stop mxs="_2:i" offset="75%" stop-color="#0BED00" stop-opacity="1"></stop><stop mxs="_2:j" offset="88%" stop-color="#FFFF00" stop-opacity="1"></stop><stop mxs="_2:k" offset="100%" stop-color="#FF0000" stop-opacity="1"></stop></lineargradient></defs><rect x="0" y="0" width="100%" height="100%" fill="url(#ghsv_' + ($expr = '<%=id%>', $e(id)) + ')"></rect></svg><div class="__mx-color_index_-slide-indicator" id="si_' + ($expr = '<%=id%>', $e(id)) + '"></div></div></div>';
    $expr = '<%if (alpha) {%>';
    if (alpha) {
        ;
        $p += '<div mxa="_2:e" class="__mx-color_index_-alpha __mx-style_index_-pr"><div class="__mx-color_index_-alpha-tracker __mx-style_index_-pr __mx-style_index_-cp" id="at_' + ($expr = '<%=id%>', $e(id)) + '" mx-mousedown="' + $viewId + '@{alpha.drag}()"><div class="__mx-color_index_-alpha-indicator" id="ai_' + ($expr = '<%=id%>', $e(id)) + '"></div></div></div>';
        $expr = '<%}%>';
    }
    ;
    $p += '<div mxv mxa="_2:f" class="__mx-color_index_-foot"><span class="__mx-color_index_-bgcolor" id="bc_' + ($expr = '<%=id%>', $e(id)) + '"></span><input class="__mx-style_index_-input __mx-color_index_-color-value" readonly="readonly" id="v_' + ($expr = '<%=id%>', $e(id)) + '"/>';
    $expr = '<%if (btns) {%>';
    if (btns) {
        ;
        $p += '<button mxs="_2:l" class="__mx-style_index_-btn __mx-style_index_-btn-brand" type="button" mx-click="' + $viewId + '@{enter}();">确定</button>';
        $expr = '<%}%>';
    }
    ;
    $p += '</div>';
