...
module.exports = Magix.View.extend({
    ...
    if ($$ref[k = $g + f] === v)
        return k; $$ref[k = $g + $$ref[$g]++] = v; return k; }, viewId = $$.viewId, dateInfo = $$.dateInfo, show = $$.show, left = $$.left, top = $$.top; var $expr, $art, $line; try {
    $p += '<div class="input _zs_gallery_mx-calendar_rangepicker_-rangepicker-input" id="input_';
    $line = 1;
    $art = '=viewId';
    ;
    $p += '' + ($expr = '<%=viewId%>', $e(viewId)) + '" mx-click="' + $viewId + '@{toggle}()" mx-change="' + $viewId + '@{stop}()"><div mxa="_zs_galleryV:_" class="_zs_gallery_mx-calendar_rangepicker_-result">';
    $line = 5;
    ...
    if (dateInfo.prefix) {
        ;
        $p += '<span mxa="_zs_galleryV:a" class="_zs_gallery_mx-calendar_rangepicker_-result-prefix">';
        $line = 6;
        $art = '=dateInfo.prefix';
        ;
    ...
    $art = '@dateInfo';
    ;
    $p += '' + ($expr = '<%@dateInfo%>', $i(dateInfo)) + '" mx-change="' + $viewId + '@{date.picked}()" mx-cancel="' + $viewId + '@{hide}()"></div>';
}
catch (ex) {
