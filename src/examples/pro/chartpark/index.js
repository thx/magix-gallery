...
var BarGraphs = function (_GraphsBase) {
    ...
    createClass(BarGraphs, [{
        ...
        value: function selectAt(ind) {
            if (_$16.indexOf(this.select.inds, ind) > -1) return;
...
var projectTheme = ["#4d7fff", "#13c9ce", "#fb734f", "#50c3f2", "#b598f1"];if (projectTheme && projectTheme.length) {
    theme.set(projectTheme);
}var chartx = { canvax: canvax, create: function create(el, _data, _opt) {
        var chart = null;var me = this;var data = canvax._.clone(_data);var opt = canvax._.extend(true, {}, _opt);var chart_id = canvax.$.query(el).getAttribute("chart_id");if (chart_id != undefined) {
            var _chart = me.instances[chart_id];if (_chart) {
                _chart.destroy();
            }delete me.instances[chart_id];
        }var Coord$$1 = Coord;if (opt.coord && opt.coord.type) {
            Coord$$1 = coord[opt.coord.type];
        }try {
            chart = new Coord$$1(el, data, opt, graphs, components);if (chart) {
                chart.draw();me.instances[chart.id] = chart;chart.on("destroy", function () {
                    me.instances[chart.id] = null;delete me.instances[chart.id];
                });
            }        } catch (err) {
