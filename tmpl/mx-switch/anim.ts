...
export default View.extend({
    ...
    '@{anim}'(state, callback) {
        let icon = $(`#${this.id} .@anim.less:switch-icon`);
        icon.addClass(state ? '@anim.less:open-icon' : '@anim.less:close-icon');
        icon.off('animationend').on('animationend', () => {
            icon.removeClass('@anim.less:open-icon @anim.less:close-icon');
