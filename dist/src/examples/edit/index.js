/*
    ...
    loader: cmd_es
 */
define("examples/edit/index",["magix","./theme","$","mx-form/index","mx-form/validator","mx-dialog/index","mx-color/picker","mx-popover/index"],(require,exports,module)=>{
...
module.exports = Base.extend({
...
} ; var $g = '', $_temp, $p = '', themes = $$.themes, itemWidth = $$.itemWidth, custom = $$.custom, cur = $$.cur, list = $$.list, viewId = $$.viewId; var $expr, $art, $line; try {
    $p += '<div mxs="_zs_galleryf:_" class="tip-content has-border has-highlight mb20 mr20"><div class="clearfix lh24"><div class="fl">操作说明：</div><div class="fl"><div>1. 选择预置主题（钻展/直通车/超级推荐）或者自定义一个颜色作为项目品牌色；</div><div>2. 系统根据品牌色计算出默认色值，可在此基础上进行调整；</div><div>3. 左侧编辑区域列举可设置的变量，支持修改</div><div>4. 右侧iframe嵌入脚手架列举常见场景，与左侧设置的变量实时联动，帮您真实复原应用场景；</div></div></div><div class="clearfix lh24 _zs_gallery_examples_edit_theme_-tip"><div class="_zs_gallery_examples_edit_theme_-tip-left">关于项目配置和组件配置的说明：</div><div>超级推荐仿照Facebook，趋于将组件颜色统一化，只通过导航颜色区分不同品牌，后续只需要配置项目色值，组件色值所有都统一。</div><div>对于老项目项目品牌色 = 组件品牌色，老项目要做成品牌色变更是比较难的，故过渡阶段兼容配置，项目品牌和组件品牌可分别设置。</div><div>单个操作项说明可hover名称查看</div></div></div><div mxa="_zs_galleryf:_" class="clearfix mb30">';
    $line = 19;
    $art = 'each themes as t i';
    ;
    $expr = '<%for (var i = 0, $art_cctyakfgio$art_c = themes.length; i < $art_cctyakfgio$art_c; i++) {    var t = themes[i]%>';
    for (var i = 0, $art_cctyakfgio$art_c = themes.length; i < $art_cctyakfgio$art_c; i++) {
        var t = themes[i];
        $p += '<div class="_zs_gallery_examples_edit_theme_-theme" style="width: ';
        $line = 20;
        $art = '=itemWidth';
        ;
        $p += ($expr = '<%=itemWidth%>', $e(itemWidth)) + 'px;"><div><a href="javascript:;" class="_zs_gallery_examples_edit_theme_-dot" style="';
        $line = 21;
        $art = '=t.styles';
        ;
    ...
    $art = '=itemWidth';
    ;
    $p += ($expr = '<%=itemWidth%>', $e(itemWidth)) + 'px;"><div mx-change="' + $viewId + 'changeColor()" mx-view="mx-color/picker?dot=true&color=';
    $line = 30;
    $art = '=custom';
    ;
    $p += ($expr = '<%!$eu(custom)%>', $eu(custom)) + '"></div><div mxs="_zs_galleryf:a" class="text-center mt10">自定义</div></div><a mxs="_zs_galleryf:b" href="javascript:;" class="fl ml20 btn btn-brand" mx-click="' + $viewId + 'copy()">复制我的定制主题</a></div><div mxv class="_zs_gallery_examples_edit_theme_-edit" style="padding-left: ';
    $line = 38;
    $art = '=(itemWidth * 4)';
    ;
    $p += ($expr = '<%=(itemWidth * 4)%>', $e((itemWidth * 4))) + 'px;"><div class="_zs_gallery_examples_edit_theme_-nav" style="left:';
    $line = 39;
    $art = '=(itemWidth*cur)';
    ;
    $p += ($expr = '<%=(itemWidth * cur)%>', $e((itemWidth * cur))) + 'px; width: ';
    $line = 39;
    $art = '=itemWidth';
    ;
    $p += ($expr = '<%=itemWidth%>', $e(itemWidth)) + 'px;"><span mxs="_zs_galleryf:c" class="_zs_gallery_examples_edit_theme_-arrow"></span><span mxs="_zs_galleryf:d" class="_zs_gallery_examples_edit_theme_-white-arrow"></span></div><div mxv class="_zs_gallery_examples_edit_theme_-colors" style="width: ';
    $line = 43;
    $art = '=(itemWidth * 4)';
    ;
    $p += ($expr = '<%=(itemWidth * 4)%>', $e((itemWidth * 4))) + 'px;">';
    $line = 44;
    $art = 'each list as group';
    ;
    $expr = '<%for (var $art_iaywxbd$art_i = 0, $art_cepzwkdi$art_c = list.length; $art_iaywxbd$art_i < $art_cepzwkdi$art_c; $art_iaywxbd$art_i++) {    var group = list[$art_iaywxbd$art_i]%>';
    for (var $art_iaywxbd$art_i = 0, $art_cepzwkdi$art_c = list.length; $art_iaywxbd$art_i < $art_cepzwkdi$art_c; $art_iaywxbd$art_i++) {
        var group = list[$art_iaywxbd$art_i];
        $p += '<div mxa="_zs_galleryf:f" class="_zs_gallery_examples_edit_theme_-group">';
        $line = 46;
        $art = '=group.text';
        ;
        ...
        ;
        $expr = '<%for (var $art_ihozmvtpjxc$art_i = 0, $art_objhilgymkexu$art_obj = group.subs, $art_ckssveab$art_c = $art_objhilgymkexu$art_obj.length; $art_ihozmvtpjxc$art_i < $art_ckssveab$art_c; $art_ihozmvtpjxc$art_i++) {        var item = $art_objhilgymkexu$art_obj[$art_ihozmvtpjxc$art_i]%>';
        for (var $art_ihozmvtpjxc$art_i = 0, $art_objhilgymkexu$art_obj = group.subs, $art_ckssveab$art_c = $art_objhilgymkexu$art_obj.length; $art_ihozmvtpjxc$art_i < $art_ckssveab$art_c; $art_ihozmvtpjxc$art_i++) {
            var item = $art_objhilgymkexu$art_obj[$art_ihozmvtpjxc$art_i];
            $p += '<div mxv mxa="_zs_galleryf:h" class="_zs_gallery_examples_edit_theme_-item"><div mxa="_zs_galleryf:i" class="_zs_gallery_examples_edit_theme_-name mb5"><div mxa="_zs_galleryf:j" class="color-6">';
            $line = 56;
            $art = '=item.text';
            ;
            $p += ($expr = '<%=item.text%>', $e(item.text)) + '<span class="mc-iconfont _zs_gallery_examples_edit_theme_-name-icon" mx-view="mx-popover/index?content=';
            $line = 57;
            $art = '=item.tip';
            ;
            ...
            $art = '@{left: 80}';
            ;
            $p += ($expr = '<%@{ left: 80 }%>', $i($$ref, { left: 80 })) + '">&#xe629;</span></div><div mxa="_zs_galleryf:k" class="_zs_gallery_examples_edit_theme_-name-key">';
            $line = 60;
            $art = '=item.key';
            ;
            ...
            if (item.type == 'color') {
                ;
                $p += '<div mxv class="_zs_gallery_examples_edit_theme_-color-wrapper" style="background-color: ';
                $line = 63;
                $art = '=item.value';
                ;
                $p += ($expr = '<%=item.value%>', $e(item.value)) + ';"><div mxv="list" mxe="' + $viewId + '_0_' + ($expr = '<%=$art_iaywxbd$art_i%>', $e($art_iaywxbd$art_i)) + '_' + ($expr = '<%=$art_ihozmvtpjxc$art_i%>', $e($art_ihozmvtpjxc$art_i)) + '" mxc="[';
                $line = 65;
                $art = ':item.value';
                ;
                $p += '{p:\'list.' + ($expr = '<%=$art_iaywxbd$art_i%>', $e($art_iaywxbd$art_i)) + '.subs.' + ($expr = '<%=$art_ihozmvtpjxc$art_i%>', $e($art_ihozmvtpjxc$art_i)) + '.value\',a:\'color\'}]" mx-change="' + $viewId + 'sync({key:\'';
                $line = 66;
                $art = '=item.key';
                ;
            ...
            if (item.type == 'text') {
                ;
                $p += '<div mxv><input mxe="' + $viewId + '_1_' + ($expr = '<%=$art_iaywxbd$art_i%>', $e($art_iaywxbd$art_i)) + '_' + ($expr = '<%=$art_ihozmvtpjxc$art_i%>', $e($art_ihozmvtpjxc$art_i)) + '" mxc="[';
                $line = 72;
                $art = ':item.value';
                ;
                $p += '{p:\'list.' + ($expr = '<%=$art_iaywxbd$art_i%>', $e($art_iaywxbd$art_i)) + '.subs.' + ($expr = '<%=$art_ihozmvtpjxc$art_i%>', $e($art_ihozmvtpjxc$art_i)) + '.value\'}]" class="w100" value="';
                $line = 72;
                $art = ':item.value';
                ;
            ...
            if (item.gap) {
                ;
                $p += '<div mxs="_zs_galleryf:e" class="_zs_gallery_examples_edit_theme_-item"></div>';
                $line = 79;
                $art = '/if';
                ;
                $expr = '<%}%>';
    ...
    ;
    $p += '</div><div mxa="_zs_galleryf:l" class="_zs_gallery_examples_edit_theme_-preview"><iframe src="https://mo.m.taobao.com/page_201904221752051#!/example/second1" class="_zs_gallery_examples_edit_theme_-iframe" sandbox="allow-forms allow-popups allow-pointer-lock allow-same-origin allow-scripts" id="frame_';
    $line = 87;
    $art = '=viewId';
    ;
    ...
    msg += $expr + '\r\n\tat file:examples/edit/index.html';
    throw msg;
} return $p; },
    ...
    'copy<click>': function (event) {
        ...
        this.mxModal('examples/edit/copy', {
            colors: colors
        }, {
            header: {
                title: '复制我的定制主题'
