...
exports["default"] = index_1["default"].extend({
    ...
    $p += '<div mxa="_zs_galleryal:_" class="clearfix" mx-mouseout="' + $viewId + 'out()">';
    $line = 3;
    $art = 'each groups as list gIndex';
    ;
    $expr = '<%for (var gIndex = 0, $art_cmagqeubim$art_c = groups.length; gIndex < $art_cmagqeubim$art_c; gIndex++) {    var list = groups[gIndex]%>';
    for (var gIndex = 0, $art_cmagqeubim$art_c = groups.length; gIndex < $art_cmagqeubim$art_c; gIndex++) {
        ...
        if (gIndex == 0) {
            ...
            $art = '=width';
            ;
            $p += ($expr = '<%=width%>', $e(width)) + 'px;"><div mxa="_zs_galleryal:a" class="clearfix" style="padding: 10px 24px;border-bottom: 1px solid var(--color-border)"><div mxa="_zs_galleryal:b" style="float:left; height: 32px; line-height: 32px;"><span mxa="_zs_galleryal:c" class="grid-title" style="margin-right: 16px;">';
            $line = 7;
            $art = '=title';
            ;
            $p += ($expr = '<%=title%>', $e(title)) + '</span><span mxa="_zs_galleryal:d" style="margin-right: 16px; color: #999;">';
            $line = 7;
            $art = '=tip';
            ;
            ...
            ;
            $expr = '<%for (var iIndex = 0, $art_cjmgwpw$art_c = list.length; iIndex < $art_cjmgwpw$art_c; iIndex++) {            var item = list[iIndex]%>';
            for (var iIndex = 0, $art_cjmgwpw$art_c = list.length; iIndex < $art_cjmgwpw$art_c; iIndex++) {
                ...
                $art = '=iIndex';
                ;
                $p += ($expr = '<%=iIndex%>', $e(iIndex)) + '})"><div mxa="_zs_galleryal:e" class="_zs_gallery_mx-cascade_card_-img"><img src="';
                $line = 13;
                $art = '=item[imgKey]';
                ;
                $p += ($expr = '<%=item[imgKey]%>', $e(item[imgKey])) + '"/></div><div mxa="_zs_galleryal:f" class="_zs_gallery_mx-cascade_card_-cont"><div mxa="_zs_galleryal:g" class="_zs_gallery_mx-cascade_card_-text">';
                $line = 15;
                $art = '=item[textKey]';
                ;
                $p += ($expr = '<%=item[textKey]%>', $e(item[textKey])) + '</div><div mxa="_zs_galleryal:h" class="_zs_gallery_mx-cascade_card_-tip">';
                $line = 16;
                $art = '=item[tipKey]';
                ;
                ...
                if (item.children && item.children.length) {
                    ;
                    $p += '<span mxs="_zs_galleryal:_" class="mc-iconfont _zs_gallery_mx-cascade_card_-arrow">&#xe692;</span>';
                    $line = 20;
                    $art = '/if';
                    ;
                    $expr = '<%}%>';
            ...
            $art = 'each list as item iIndex';
            ;
            $expr = '<%for (var iIndex = 0, $art_cytzvjogrl$art_c = list.length; iIndex < $art_cytzvjogrl$art_c; iIndex++) {            var item = list[iIndex]%>';
            for (var iIndex = 0, $art_cytzvjogrl$art_c = list.length; iIndex < $art_cytzvjogrl$art_c; iIndex++) {
                var item = list[iIndex];
                $p += '<div class="_zs_gallery_mx-cascade_card_-line ';
                $line = 29;
                $art = 'if item.cur';
                ...
                $art = '=iIndex';
                ;
                $p += ($expr = '<%=iIndex%>', $e(iIndex)) + '})"><span mxa="_zs_galleryal:i" class="_zs_gallery_mx-cascade_card_-text">';
                $line = 32;
                $art = '=item[textKey]';
                ;
