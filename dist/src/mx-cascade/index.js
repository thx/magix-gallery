...
module.exports = Magix.View.extend({
    ...
    ;
    $p += '><span mxa="_zs_galleryap:_" class="mx-trigger-label">';
    $line = 3;
    $art = 'if name';
    ;
    $expr = '<%if (name) {%>';
    ...
    $art = '=selectedText';
    ;
    $p += ($expr = '<%=selectedText%>', $e(selectedText)) + '</span><span mxs="_zs_galleryap:_" class="mc-iconfont mx-trigger-arrow">&#xe692;</span></div>';
    $line = 6;
    $art = 'if !disabled';
    ;
    $expr = '<%if (!disabled) {%>';
    if (!disabled) {
        ...
        ;
        $expr = '<%for (var gIndex = 0, $art_ccycwstivtwi$art_c = groups.length; gIndex < $art_ccycwstivtwi$art_c; gIndex++) {        var list = groups[gIndex]%>';
        for (var gIndex = 0, $art_ccycwstivtwi$art_c = groups.length; gIndex < $art_ccycwstivtwi$art_c; gIndex++) {
            ...
            ;
            $expr = '<%for (var iIndex = 0, $art_cgfltemqtpw$art_c = list.length; iIndex < $art_cgfltemqtpw$art_c; iIndex++) {            var item = list[iIndex]%>';
            for (var iIndex = 0, $art_cgfltemqtpw$art_c = list.length; iIndex < $art_cgfltemqtpw$art_c; iIndex++) {
