...
module.exports = Magix.View.extend({
    ...
    ;
    $expr = '<%for (var index = 0, $art_ctlaszybzu$art_c = list.length; index < $art_ctlaszybzu$art_c; index++) {    var item = list[index]%>';
    for (var index = 0, $art_ctlaszybzu$art_c = list.length; index < $art_ctlaszybzu$art_c; index++) {
        var item = list[index];
        $p += '<div mxv mxa="_zs_galleryfL:_" class="_zs_gallery_mx-tree_index_-li"><div mxv mxa="_zs_galleryfL:a" class="_zs_gallery_mx-tree_index_-name clearfix">';
        $line = 4;
        $art = 'if needExpand';
        ;
        $expr = '<%if (needExpand) {%>';
        if (needExpand) {
            ...
            if (item.children && (item.children.length > 0)) {
                ...
                if (readOnly) {
                    ;
                    $p += '<i mxs="_zs_galleryfL:_" class="mc-iconfont _zs_gallery_mx-tree_index_-icon">&#xe732;</i>';
                    $line = 13;
                    $art = '/if';
                    ;
                    $expr = '<%}%>';
        ...
        ;
        $p += '<label mxv mxa="_zs_galleryfL:b" class="fl">';
        $line = 17;
        $art = 'if !readOnly';
        ;
        $expr = '<%if (!readOnly) {%>';
