...
module.exports = Magix.View.extend({
    ...
    ;
    $p += '><span mxa="_zs_galleryf_:_" class="mx-trigger-label">';
    $line = 4;
    $art = 'if name';
    ;
    $expr = '<%if (name) {%>';
    if (name) {
        ;
        $p += '<span mxa="_zs_galleryf_:a" class="color-9">';
        $line = 4;
        $art = '=name';
        ;
    ...
    $art = '=time';
    ;
    $p += ($expr = '<%=time%>', $e(time)) + '</span><span mxs="_zs_galleryf_:_" class="mc-iconfont mx-trigger-arrow">&#xe692;</span></div><div class="mx-output mx-output-bottom ';
    $line = 10;
    $art = 'if expand';
    ;
    $expr = '<%if (expand) {%>';
    ...
    if (rList) {
        ...
        $art = '=viewId';
        ;
        $p += ($expr = '<%=viewId%>', $e(viewId)) + '_content" mx-change="' + $viewId + '@{stop}()"></div><div mxs="_zs_galleryf_:a" class="mx-output-footer clearfix"><a href="javascript:;" class="fl btn btn-small btn-brand" mx-click="' + $viewId + '@{hide}({enter:true})">确定</a><a href="javascript:;" class="fl btn btn-small ml10" mx-click="' + $viewId + '@{hide}()">取消</a></div>';
        $line = 21;
        $art = '/if';
        ;
        $expr = '<%}%>';
