...
module.exports = Magix.View.extend({
    ...
    if (!items.length) {
        ;
        $p += '<span mxa="_zs_gallerye|:_" class="_zs_gallery_mx-taginput_index_-placeholder">';
        $line = 6;
        $art = '=placeholder';
        ;
    ...
    if (items.length) {
        ...
        ;
        $expr = '<%for (var idx = 0, $art_crrpvslcuoc$art_c = items.length; idx < $art_crrpvslcuoc$art_c; idx++) {        var one = items[idx]%>';
        for (var idx = 0, $art_crrpvslcuoc$art_c = items.length; idx < $art_crrpvslcuoc$art_c; idx++) {
            var one = items[idx];
            $p += '<span mxa="_zs_gallerye|:a" class="_zs_gallery_mx-taginput_index_-item clearfix" mx-contextmenu="' + $viewId + 'prevent()">';
            $line = 12;
            $art = '=one.text';
            ;
    ...
    if (!disabled) {
        ...
        ;
        $p += '" mx-mouseout="' + $viewId + '@{out}()" mx-contextmenu="' + $viewId + '@{prevent}()"><ul mxa="_zs_gallerye|:b" class="mx-output-list">';
        $line = 36;
        $art = 'if loading';
        ;
        $expr = '<%if (loading) {%>';
        if (loading) {
            ;
            $p += '<li mxs="_zs_gallerye|:_"><div class="loading" style="padding: 40px 0;"><span class="loading-anim"></span></div></li>';
            $line = 40;
            $art = 'else';
            ;
            $expr = '<%}    else {%>';
            ...
            if (suggest && suggest.length) {
                ...
                ;
                $expr = '<%for (var idx = 0, $art_cdgsdslrhto$art_c = suggest.length; idx < $art_cdgsdslrhto$art_c; idx++) {                var item = suggest[idx]%>';
                for (var idx = 0, $art_cdgsdslrhto$art_c = suggest.length; idx < $art_cdgsdslrhto$art_c; idx++) {
                ...
                ;
                $p += '<li mxa="_zs_gallerye|:c" class="text-center color-9 pt20 pb20">';
                $line = 51;
                $art = '=emptyText';
                ;
