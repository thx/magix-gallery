...
module.exports = Magix.View.extend({
...
} ; var $g = '', $_temp, $p = '', searchbox = $$.searchbox, text = $$.text, keyword = $$.keyword, spm = $$.spm, multiple = $$.multiple, allHide = $$.allHide, height = $$.height, parents = $$.parents, hasGroups = $$.hasGroups; var $expr, $art, $line; try {
    $line = 1;
    $art = 'if searchbox';
    ;
    $expr = '<%if (searchbox) {%>';
    if (searchbox) {
        ;
        $p += '<div mxv mxa="_zs_gallerya|:_" class="mx-output-search"><div mxv mxa="_zs_gallerya|:a" class="search-box" style="width: 100%;"><i mxs="_zs_gallerya|:_" class="mc-iconfont search-icon">&#xe651;</i><input class="input search-input" placeholder="';
        $line = 6;
        $art = '=text.search';
        ;
        $p += ($expr = '<%=text.search%>', $e(text.search)) + '" value="';
        $line = 7;
        $art = '=keyword';
        ;
        $p += ($expr = '<%=keyword%>', $e(keyword)) + '" mx-keyup="' + $viewId + '@{search}()" mx-paste="' + $viewId + '@{search}()" mx-change="' + $viewId + '@{stop}()" mx-focusin="' + $viewId + '@{stop}()" mx-focusout="' + $viewId + '@{stop}()" ';
        $line = 13;
        $art = 'if spm';
        ;
        $expr = '<%if (spm) {%>';
    ...
    if (multiple) {
        ;
        $p += '<div mxa="_zs_gallerya|:b" class="_zs_gallery_mx-dropdown_index_-oper-wrapper _zs_gallery_mx-dropdown_index_-has-group clearfix"><a class="_zs_gallery_mx-dropdown_index_-oper" href="javascript:;" mx-click="' + $viewId + '@{checkAll}({checked:true})" ';
        $line = 19;
        $art = 'if spm';
        ;
        $expr = '<%if (spm) {%>';
        ...
        $art = '=text.select';
        ;
        $p += ($expr = '<%=text.select%>', $e(text.select)) + '</a><a class="_zs_gallery_mx-dropdown_index_-oper" href="javascript:;" mx-click="' + $viewId + '@{checkAll}({checked:false})" ';
        $line = 20;
        $art = 'if spm';
        ;
        $expr = '<%if (spm) {%>';
    ...
    if (!allHide) {
        ...
        ;
        $expr = '<%for (var parentIndex = 0, $art_cbdeltyhxpp$art_c = parents.length; parentIndex < $art_cbdeltyhxpp$art_c; parentIndex++) {        var parent = parents[parentIndex]%>';
        for (var parentIndex = 0, $art_cbdeltyhxpp$art_c = parents.length; parentIndex < $art_cbdeltyhxpp$art_c; parentIndex++) {
            ...
            if (!parent.hide) {
                ...
                ;
                $expr = '<%for (var itemIndex = 0, $art_objeszypcrmi$art_obj = parent.list, $art_civgnzmxtog$art_c = $art_objeszypcrmi$art_obj.length; itemIndex < $art_civgnzmxtog$art_c; itemIndex++) {                var item = $art_objeszypcrmi$art_obj[itemIndex]%>';
                for (var itemIndex = 0, $art_objeszypcrmi$art_obj = parent.list, $art_civgnzmxtog$art_c = $art_objeszypcrmi$art_obj.length; itemIndex < $art_civgnzmxtog$art_c; itemIndex++) {
                    var item = $art_objeszypcrmi$art_obj[itemIndex];
                    $p += ' ';
                    $line = 31;
                    $art = 'if !item.hide';
                    ;
                    ...
                    if (!item.hide) {
                        ...
                        if (multiple) {
                            ...
                            $art = '=itemIndex';
                            ;
                            $p += ($expr = '<%=$eq(itemIndex)%>', $e($eq(itemIndex))) + '\'})" ';
                            $line = 40;
                            $art = 'if spm';
                            ;
                            $expr = '<%if (spm) {%>';
    ...
    if (multiple) {
        ;
        $p += '<div mxa="_zs_gallerya|:d" class="mx-output-footer"><a mxa="_zs_gallerya|:e" href="javascript:;" class="btn btn-small btn-brand mr10" mx-click="' + $viewId + '@{submit}()">';
        $line = 60;
        $art = '=text.submit';
        ;
