...
module.exports = Base.extend({
    ...
    if (loading) {
        ;
        $p += '<button mxs="_zs_gallery5:a" type="button" class="mx-btn btn-disabled w200" disabled="true"><span class="mx-btn-loading-circle"></span>loading禁用，2s后释放</button>';
        $line = 10;
        $art = 'else';
        ;
        $expr = '<%}else {%>';
        ...
        ;
        $p += '<button mxs="_zs_gallery5:b" type="button" class="mx-btn btn-brand w200" mx-click="' + $viewId + 'change()">点我切换成loading禁用状态</button>';
        $line = 14;
        $art = '/if';
        ;
        $expr = '<%}%>';
